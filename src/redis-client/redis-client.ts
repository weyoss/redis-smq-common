...
export class RedisClient extends EventEmitter {
  ...
  protected constructor(config: TRedisConfig) {
    super();
  ...
  protected getClient(config: TRedisConfig): TCompatibleRedisClient {
    // in the javascript land, we can pass any value
    if (!Object.values(RedisClientName).includes(config.client)) {
...
RedisClient.addScript(
  ELuaScriptName.ZPOPRPUSH,
  fs.readFileSync(`${__dirname}/lua/zpoprpush.lua`).toString(),
);
RedisClient.addScript(
  ELuaScriptName.ZPOPHGETRPUSH,
...
RedisClient.addScript(
  ELuaScriptName.ZPUSHHSET,
  fs.readFileSync(`${__dirname}/lua/zpushhset.lua`).toString(),
);
RedisClient.addScript(
  ELuaScriptName.LPOPRPUSH,
